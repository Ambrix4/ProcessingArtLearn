...
class Particle{
  ...
  void update(int[] k,int ct){
    //TAU可以表示方向，noise可以看作是某种和位置相关的随机特性
    //即很多点到这个位置之后都会出现一样的noise,柏林噪声是连续的，放大尺幅
    ff = noise(pos.x/100/ct, pos.y/100/ct) * TWO_PI; // Flow Field
    //ff=(0,1)*2pi
    float mainP = 1200;
    ...
    ////改变颜色
    if (ff < 6 && ff > 3) {//右下角流动
    //if(red(k[int(pos.y)*1920+int(pos.x)]) >100){
      c = colors[0];//红线
      stroke(c);
      pos.add(tan(ff)*random(1,10), tan(ff));//移动的距离x更大一些
  ...
  void show(){
    ...
    float lx = 10;//左边距
    float ly = 10;//上边距
    pos.x = constrain(pos.x, lx, width - lx);//约束框
    pos.y = constrain(pos.y, ly, height - ly);
    point(pos.x, pos.y);
  };
